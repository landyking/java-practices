...
脚本地址：`https://github.com/Supervisor/initscripts/blob/master/redhat-init-mingalevme`
...
获取脚本内容后，针对其中的`PREFIX`结合自己系统路径进行替换。

将替换后的内容保存到`/etc/init.d/supervisord`。并为其增加可执行权限`chmod a+x /etc/init.d/supervisord`。

### 添加为系统服务

使用命令添加系统服务`chkconfig --add supervisord`
