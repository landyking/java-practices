...
public class SpElUtilTest {
    ...
    public void testSpelAssignmentExpression() throws Exception {
        Date2 oldDate = spel.getTime();// 获取当前time属性值
        exp.parseExpression("time").setValue(secontext, new Date2(113, 2, 25)); // 为time属性重新赋值
        Date2 newDate = spel.getTime();// 获取赋值后的time属性值
        assertEquals(2013,
    ...
    public void testSpelConstructorsExpression() throws Exception {
        SpelTestInnerClass spt = exp
                .parseExpression(
                        "new springel.SpelTestInnerClass('constructTest',23)")
